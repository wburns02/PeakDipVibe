          ...
          {(["Strong Trend", "Pullback Zone", "Bear Rally", "Deep Value"] as const).map((q) => {
            ...
            return (
              <div key={q} className={`rounded-lg border p-2.5 text-center ${colors[q]}`}>
                ...
                <p className="text-[10px] font-medium uppercase tracking-wider">{q}</p>
                <p className="text-[10px] opacity-70">{pct}% of market</p>
              </div>
            );
          })}
        </div>
      ...
      <Card className="overflow-hidden">
        <div className="mb-2 flex items-center justify-between">
          <div>
            ...
            <p className="text-[10px] text-text-muted">Click any dot to view stock detail. Dot size = daily change magnitude.</p>
          </div>
          {highlighted && (
        ...
        <div className="mt-3 grid grid-cols-2 gap-2 text-center text-[10px] font-medium md:grid-cols-4">
          ...
          return (
            <Card key={q} className={c.border}>
              <div className="mb-3 flex items-center justify-between">
                ...
                <span className="text-[10px] text-text-muted">{filteredPoints.filter((p) => p.quadrant === q).length} stocks</span>
              </div>
              {qStocks.length === 0 ? (
                ...
                <div className="space-y-1">
                  {qStocks.map((s) => (
                    ...
                    >
                      ...
                      <span className="text-[10px] text-text-muted">RSI {s.rsi.toFixed(0)}</span>
